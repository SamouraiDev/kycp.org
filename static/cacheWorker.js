...
class CacheWorker {
  ...
  _getCache(name) {
    return windowCaches.open(name)
  }

  get(name, key) {
    console.log('worker:get')
    return this._getCache(name).then(cache => cache.match(key).then(response => response ? response.json() : undefined))
  }
  put(name, key, value) {
    const jsonStr = JSON.stringify(value)
    return this._getCache(name).then(cache => cache.put(key, new Response(jsonStr, {headers:{'content-type':'application/json'}})))
  }
  keys(name) {
    return this._getCache(name).then(cache => cache.keys())
  }
  delete(name, key) {
    return this._getCache(name).then(cache => cache.delete(key))
  }
  clear(name) {
    return windowCaches.delete(name)
  }
  clearAll() {
    windowCaches.keys().then(names => names.forEach(name => windowCaches.delete(name)))
  }
}

console.log('CacheWorker: ready')
expose(CacheWorker, self)
